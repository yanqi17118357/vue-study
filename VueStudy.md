...
每一个Vue应用都是通过 createApp 函数创建一个新的应用实例：
...
传入createApp的对象实际上是一个组件，每个应用都需要一个“根组件”，其他组件作为子组件
...
该方法接收一个容器参数，可以是一个实际的DOM元素或是一个CSS选择器字符串：
...
双大括号标签会被替换为相应组件实例中msg属性的值，随msg属性更改时更新
...
这是Vue提供的特殊 attribute，它们将为渲染的DOM 应用特殊的响应式行为。

这里我们做的事情简单来说就是：在当前组件实例上，将此元素的 innerHTML 与 `rawHtml` 属性保持同步。

`span` 的内容将会被替换为 `rawHtml` 属性的值，插值为纯 HTML——数据绑定将会被忽略。注意，你不能使用 `v-html` 来拼接组合模板，因为 Vue 不是一个基于字符串的模板引擎。在使用 Vue 时，应当使用组件作为 UI 重用和组合的基本单元。

![image-20230411125927144](C:\Users\32248\AppData\Roaming\Typora\typora-user-images\image-20230411125927144.png)
...
```html
<button :disable="isButtonDisabled">
    Button
</button>
```
...
如果你有像这样的一个包含多个 attribute 的 JS 对象：
...
![指令语法](https://cn.vuejs.org/assets/directive.69c37117.png)
...
这确保了方法在作为事件监听器或回调函数时始终保持正确的 `this`。你不应该在定义 `methods` 时使用箭头函数，因为箭头函数没有自己的 `this` 上下文。
...
然而有些情况下，我们需要在状态变化时执行一些操作：如更改DOM，或是根据异步操作的结果去修改另一处的状态
...
默认情况下，用户创建的侦听器回调会在 Vue 组件更新之前被调用，这意味着你在侦听器回调访问的DOM将是被Vue更新之前的状态

如果想在侦听器回调中能被访问被Vue更新之后的DOM，你需要指明，`flush: 'post'` 选项
...
让我们继续关注我们的 `<BlogPost>` 组件。我们会发现有时候它需要与父组件进行交互。
...
![image-20230413201609169](C:\Users\32248\AppData\Roaming\Typora\typora-user-images\image-20230413201609169.png)
...
由于浏览器的原生 HTML 解析行为限制，有一些需要注意的事项。
